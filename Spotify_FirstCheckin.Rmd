---
title: 'Mid Term Check-in: Spotify'
---
...
* Build a K -means model to identify the most popular songs by each cluster:  
  ...
  + Understand how the audio features  affect the songs group wise.
...
* **Exploratory Data Analysis**
  ...
  + Statistical Testing to understand the variable behavior
  
* **K - Means Clustering**
  + To understand the popularity of songs I used K-means clustering method, found the centroid of each cluster and identified how far these clusters are from each other.
  + Songs with similar characteristics are grouped into clusters by the algorithm and these clusters help in understanding the audio attributes behavior.
...
* **viridis** : Similar to Rcolorbrewer, helps with color palettes and cosmetic purposes
...
#### About Data {.tabset .tabset-fade .tabset-pills}
...
The following information about the variables is provided on the ['rfordatascience'](https://github.com/rfordatascience/tidytuesday/blob/master/data/2020/2020-01-21/readme.md) website and helps users to understand the dataset:
...
* Then I identified rows with missing values in the 3 columns.
...
* This is a very small number of missing values in a large dataset, and hence it is not detrimental to the analysis, and therefore I will be omitting them.
```{r}
...
* Now, the variables Track_id, track_album_id, track_album_name, sub genre, duration are not important to the analysis and hence I have dropped them.
```{r}
...
```{r hist, warning=FALSE,echo=TRUE}
...
spotify %>%
  ...
  ggtitle("Distribution of Attributes") + 
...
Now, to understand genres better, I plotted genres by their average popularity.
...
To understand keys & mode better, I did chi square test on them as they are categorical variables  

* Ho : Key is independent of mode
* Ha : Key is not indepent of mode

On applying the chisq.test function the two variables, I found the p-value to be lesser than 2.2e-16, which is too significant for an \alpha of 0.05.

Hence, the null hypothesis is to rejected.  
...
#### Approach {.tabset .tabset-fade .tabset-pills}

Approach for this section:

1. I have taken 70% of the data as training set and rest 30% as test set.
2. Next, the data was scaled to make the attributes comparable
3. Interpret the correlation plot
4. Then found the optimal centers using elbow method to implement K-Means Clustering
5. Fit the K Means Clustering Model
6. Next, found the cluster information for their attributes which were grouped by mean
7. Detect the accuracy of the model
8. Cluster wise indepth analysis
...
#### Corr Plot {.tabset .tabset-fade .tabset-pills}
...
#### K Means {.tabset .tabset-fade .tabset-pills}

**K Means Clustering Process**

K-means clustering is a simple, yet quick algorithm which deals with large data sets, like this data.

The idea behind k-means is  that grouping data into clusters such that the variation inside the clusters (also known as total within-cluster sum of square or WSS) is minimum, and within each other is maximum. This helps in understanding, which songs tend to be popular in which groups

**General K-Means Process**

1. Identify the number of clusters (K) to be created, elbow method has been used for the same
...
I have used the elbow method to find the optimum number of clusters. One reason for using this method is that it chooses the correct number of clusters over random assignment of samples to clusters.
...
The clusters are extracted and added to the data to do some descriptive statistics at the cluster level. The datatable below is a result of clustering. The right most column depicts the cluster the songs belong to, and this will help in further analysis to understand the features of the clusters.
```{r}
#Assiging cluster to df
train_kmeans$cluster <- as.factor(fit$cluster)
datatable(head(train_kmeans,5))
```

#### Quality {.tabset .tabset-fade .tabset-pills}
...
* The BSS is 51208.72.
  ...
  - The higher it is, the better it is as we want the cluster centers far apart from each other.
  - A large BSS implies that the characteristics of the clusters are unique and very obviosuly identifiable.
...
* The idea is to maximize the bss/tss%. But to get a high value, we need to increase the number of clusters.But in this case, we found the number of clusters to be ideal at 3, hence we'll stick to it.
```{r}
...
* The largest cutoff for clusters is 3, hence though we have a low bss/tss% we continue with 3 clusters.
* The prediction strength for the clusters is decent as it is above 0.7 for all clusters
```{r, warning=FALSE}
#Prediction Strength
prediction.strength(train_scale, Gmin=2, Gmax=5, M=10,cutoff=0.8)
```

#### Behavior {.tabset .tabset-fade .tabset-pills}
...
* Accousticness and energy varies drastically across the clusters. Hence it will be used in final analysis
...
#### Clust Analysis {.tabset .tabset-fade .tabset-pills}

**Individual Cluster Analysis**
For the cluster analysis I have kept a baseline for popularity as 85 and above. These are the most popular songs of each cluster.
...
n <- c1 %>% 
  ...
  subset(track_popularity >= 85 & playlist_genre %in% c("rap","rock","pop")) %>% 
  distinct(track_name,.keep_all = TRUE) 

datatable(n, options = list(order = list(list(6, 'desc'),list(5, 'desc')))) #sorted by popularity, energy
```
...
Therefore, the cluster two songs are high on energy and low on acousticness.
```{r}
n <- c2 %>% 
  select(track_name,track_artist,playlist_genre,acousticness,energy,track_popularity) %>% 
  subset(track_popularity >= 85 & playlist_genre %in% c("latin","rock","pop")) %>% 
  distinct(track_name,.keep_all = TRUE) 

datatable(n, options = list(order = list(list(6, 'desc'),list(5, 'desc'))))
```
...
Therefore, the cluster two popular songs are high on acousticness with average energy.
```{r}
n <- c3 %>% 
  select(track_name,track_artist,playlist_genre,acousticness,energy,track_popularity) %>% 
  subset(track_popularity >= 85 & playlist_genre %in% c("latin","rap","pop")) %>% 
  distinct(track_name,.keep_all = TRUE) 
...
* Through out this process we aimed to understand what makes the clusters different from each other, which also lead us to top songs in each category

* We did analysis through Visual Exploration, Statistical testing and K means clustering to arrive at the below takeways

* Key Takeways:
  - The three clusters dont vary on the popularity, but instead vary highly on energy and acousticness.
